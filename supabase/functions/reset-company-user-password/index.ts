...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (callerError || !callerMember || callerMember.role !== 'ceo') {
      return new Response(
        JSON.stringify({ error: 'Only CEO can reset passwords' }),
    ...
    // Cannot reset CEO's own password through this
    if (targetMember.role === 'ceo') {
      return new Response(
        JSON.stringify({ error: 'Cannot reset CEO password through this method' }),
        { status: 403, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }
